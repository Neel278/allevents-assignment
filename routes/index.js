...
const { isDataValidRealScenarios } = require("../helpers");
...
router.post("/insertEvent", async (req, res) => {
    // validate event data with real life scenario
    ...
    const validationResult = isDataValidRealScenarios(eventData);
    if (validationResult != false) {
        res.send(validationResult);
        return;
    }

    // if validation success add data to db
    const data = Object.values(eventData);
    try {
        await conn.query(
            "INSERT INTO `events_data`(`event_id`, `eventname`, `start_time`, `end_time`, `timezone`, `city`, `state`, `country`, `description`, `banner_url`, `score`, `categories`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            data
        );
    } catch (e) {
        console.log(e);
...
router.post("/syncData", async (req, res) => {
    ...
    let jsonData = await csvToJson.fromFile(
        __dirname + "/../files/backend_task_sample_data.csv"
    );
...
router.get("/getEventsCount", async (req, res) => {
    try {
        let data = await conn.query(
            `SELECT eventname,description FROM events_data ORDER BY score DESC`
        );
        data = data[0];
        let ans = [];
